...
- `workers` - registry worker-ов: статус и последний heartbeat.
...
Кроме task tracking, Planner обновляет worker registry: worker получает статус `idle`, `running` или `offline`. Текущая задача не дублируется в `workers`, ее можно получить из таблицы `tasks` по `status='running'` и `worker_id`.
...
WORKER_HEARTBEAT_TIMEOUT_CHECK_SECONDS = 5
```
...
- `migrations/versions/003_add_worker_registry.py`.
